...
if (isset($usuario) && $idRolUsuario === 1) {
    try {
        ...
        $nombre = isset($_POST['nombre-esteticista']) && !empty($_POST['nombre-esteticista']) ? $_POST['nombre-esteticista'] : null;  
        $idCategoria = isset($_POST['categoria-esteticista']) && !empty($_POST['categoria-esteticista']) ? $_POST['categoria-esteticista'] : null;      
        ...
        //Se crea categoria
        $datosEsteticista = [$nombre, $idCategoria, $idCuenta];
        $stmt = $pdo->prepare("INSERT INTO t_esteticistas (nombre, id_cat, id_cuenta)
        VALUES (?, ?, ?)");
        ...
        error_log("Error al crear esteticista: " . $th->getMessage() . " en linea " . $th->getLine() . " en archivo " . $th->getFile());
        echo "Error interno del servidor. por favor intenta de nuevo. <a href='../../paginas/admin/nueva_esteticista.php'>Volver</a> o informa al administrador.";
    }
}
